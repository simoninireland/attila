...
/bin/cat $source | grep -E '// PRIMITIVE' | cut -d ' ' -f 3 >$primitives
/bin/cat $source | grep -E '^[a-zA-Z0-9_]+\(\)' | cut -d ' ' -f 1 | sed -e 's/()//g' >$functions
...
/usr/X11R6/bin/paste -d ' ' $primitives $functions | mawk '{ printf("extern void %s();\n", $2); }' >>$bootstrap
...
EOF
/usr/X11R6/bin/paste -d ' ' $primitives $functions | mawk '{ printf("   compile_primitive(\"%s\", &%s, 0);\n", $1, $2); }' >>$bootstrap
cat >>$bootstrap <<EOF
}
EOF
